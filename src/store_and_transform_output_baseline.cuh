...
__device__ void  transform_output_tile(float *pOutputs, float *C_tile, float *At, int Inx, int Iny, int TileX, int TileY, int TileZ, int out_h, int out_w, int tiles_dim, int round, int in_n, int offset, int out_thread[][4], short mask, int c_tensor, int c_glb_offset){
  ...
  int idx = out_thread[round][Iny%4] + Iny/4 + offset;
...
__device__ __inline__ void store_output_tile(float4 acumm_smem[][16], float *shared_mem, int Inx, int Iny, float *C, int TileX, int TileY, int TileZ, int out_h, int out_w,  int tiles_dim, int in_n, float4 *input_frag_mem, float4* filter_frag_mem, int out_thread[][4], int access_s_out[][16], short mask){
  ...
  acumm1 = access_s_out[0][Inx%8 + (Inx/16)*8];
  acumm2 = access_s_out[1][Inx%8 + (Inx/16)*8]; 

  int offset = BN_p*4;
  int init = (Iny/4)*BN_p*16*4 + (Iny%4)*40 + Inx; 
  int acumm3 = Iny * BN_p;
  int acumm4 = BN_p*8*2;
  ...
  int c_tensor = TileZ*in_n*out_h*out_w*BK + (TileY%tiles_dim)*in_n*2 + (TileY/tiles_dim)*in_n*out_w*2 + TileX*BN + Inx;
                  
  for(int round=0; round<4; round++){

    //transformation step    
    if ( ((!round || round==1) && (Inx&15)<8) || ((round==2 || round==3) && (Inx&15)>7) ){
    ...
    transform_output_tile(C, C_tile, At, Inx, Iny, TileX, TileY, TileZ, out_h, out_w, tiles_dim, round, in_n, 0, out_thread, mask, c_tensor, c_glb_offset);
