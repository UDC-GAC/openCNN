...
__device__ __forceinline__ void store_output_tile(float4 acumm_smem[][16], float *shared_mem, int Inx, int Iny,
                                  float *C, int TileX, int TileY, int TileZ, int out_h, int out_w, 
                                  int tiles_dim, int in_n, float4 *input_frag_mem, float4* filter_frag_mem, 
                                  short mask){
  ...
  // For transposing
  t = Inx%8/2;
  acumm1 = t*18 + Inx%2 + (Inx/16)*2 + ((Inx/8)%2)*8;
  acumm2 = acumm1+4;
  ...
  int init = (Iny%4)*(16+2)*2 + Inx;
  init = init - init/((Iny%4+1)*32)*32 + Iny%4*32;
  init += (Iny/4)*BN_p*16*2;

  int c_glb_offset = in_n*out_h*out_w;
  int c_tensor = TileZ*c_glb_offset*BK + (TileY%tiles_dim)*in_n*2 + (TileY/tiles_dim)*in_n*out_w*2 + TileX*BN + (Inx%16)*2+
                ((Inx/16)*16 + (Iny%4)*4 + Iny/4)*c_glb_offset;
  c_tensor/=2; 
