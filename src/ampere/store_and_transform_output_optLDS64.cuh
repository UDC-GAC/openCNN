...
__device__ __inline__ void store_output_tile(float4 acumm_smem[][16], float  *shared_mem,
                                  float *C, int out_h, int out_w, 
  ...
  int acumm1, acumm2;
  // For transposing
  //acumm1 = access_s_out[threadIdx.x]; //* 4
  ...
  // For transformating
  int offset = BN_p; //*2/2
  int init = (threadIdx.y/4)*BN_p*16 + (threadIdx.y%4)*(32+2);
  init += (threadIdx.x/16)*8 + ((threadIdx.x/8)%2)*16 + (threadIdx.x%8); //40=(8+2)*4, 4 blocks/buffer


  int c_glb_offset = in_n*out_h*out_w;                    
  int c_tensor = blockIdx.z*c_glb_offset*BK + (blockIdx.y%tiles_dim)*in_n*2 + (blockIdx.y/tiles_dim)*in_n*out_w*2 + blockIdx.x*BN + ((threadIdx.x/8)%2)*2 + (threadIdx.x%8)*2*2 + ((threadIdx.x/16)*16 + (threadIdx.y%4)*4 + threadIdx.y/4)*c_glb_offset;
  c_tensor/=2; 
